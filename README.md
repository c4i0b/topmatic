...
Presets work as overlays: picking one in the TUI activates it immediately (no name, no editor) under its suggested name — `All` runs everything installed with no step list at all, and steps topgrade gains later are included automatically. Editing a preset-based profile works like any other, but the config keeps only what you changed: `extra_steps`/`excluded_steps` adjust the preset selection and an untouched override disappears from the file again.

Keys per screen (the footer always shows them): home `↑↓ move`, `n` new (from a preset), `e` edit, `d` delete, `r` run now (opens the live log view right away — output streams in while it runs, `x` stops the run, `esc` goes back while the dashboard badge keeps tracking `running · 42s`), `l` logs, `/` filter, `?` help overlay, `q` quit (lower or upper case; while a filter is active, `q` is just a letter). Editor: `↑↓←→ move`, `enter` toggle steps or choose a row, `ctrl+a` all, `ctrl+d` none (mark/clear the filtered steps), `tab` section, `ctrl+s` save from anywhere (`esc` asks to save when there are changes), `esc` back. Presets: `enter` choose. Logs: `enter` open, `r` refresh, `esc` back. Mouse click/scroll works and `L` toggles the activity panel. The right pane shows live details for the selected profile: schedule, countdown to next fire, timer state and the tail of the last run log.
